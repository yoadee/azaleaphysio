...
const covered = [
  ...
  { label: 'پوشش دیگر', value: 'پنج درمان دیگر، هرکدام جداگانه از پیش تأیید شده (پایین‌تر)' },
]

// جلسات از پیش تأییدشدهٔ ICBC Enhanced Care به تفکیک رشته، در ۱۲ هفتهٔ اول.
const preApproved = [
  { service: 'فیزیوتراپی', sessions: '۲۵' },
  { service: 'کایروپراکتیک', sessions: '۲۵' },
  { service: 'ماساژ درمانی', sessions: '۱۲' },
  { service: 'طب سوزنی', sessions: '۱۲' },
  { service: 'کینزیولوژی (توان‌بخشی فعال)', sessions: '۱۲' },
  { service: 'مشاوره', sessions: '۱۲' },
]
...
const faqs: Faq[] = [
  ...
  { category: 'insurance', q: 'آیا کینزیولوژی یا درمان‌های دیگر هم تحت پروندهٔ من پوشش دارد؟', a: 'اغلب بله. در کنار فیزیوتراپی، ICBC برای کایروپراکتیک، ماساژ درمانی، طب سوزنی، کینزیولوژی (توان‌بخشی فعال) و مشاوره هم جلساتی را از پیش تأیید می‌کند، هرکدام با سقف جلسهٔ خودش در ۱۲ هفتهٔ اول. چون کلینیک چندتخصصی است، فیزیوتراپیست می‌تواند موارد مناسب را در یک برنامهٔ هماهنگ بیاورد.' },
]
...
export default function FaIcbc() {
  return (
    <>
        ...
        <div className="max-w-[1100px] mx-auto grid grid-cols-1 md:grid-cols-[1.4fr_1fr] gap-12 md:gap-20">
          <div>
            ...
            <p className="text-[16px] text-muted leading-[2]">
              بازهٔ ۱۲ هفته از روز تصادف شروع می‌شود، پس هرچه زودتر نوبت بگیرید، از پوشش خود بیشتر استفاده می‌کنید و آسیب‌های بافت نرم بهتر پاسخ می‌دهند. اگر به درمان بیشتری نیاز داشته باشید، تمدید را از طرف شما درخواست می‌کنیم. ICBC برای چند درمان دیگر هم تعداد مشخصی جلسه را از پیش تأیید می‌کند (پایین‌تر فهرست شده) که همه را زیر یک سقف هماهنگ می‌کنیم.
            </p>
          </div>
      ...
      {/* Pre-approved visits by treatment */}
      <section className="bg-stone px-6 sm:px-10 md:px-14 py-20 md:py-24" style={{ borderTop: '1px solid var(--color-border)' }}>
        <div className="max-w-[1100px] mx-auto">
          <div className="max-w-[640px] mb-12 md:mb-16">
            <h2 className="text-[clamp(1.75rem,3.4vw,2.5rem)] font-light leading-[1.4] text-text mb-6">
              ICBC چه چیزی را به تفکیک درمان از پیش تأیید می‌کند
            </h2>
            <p className="text-[17px] text-muted leading-[2]">
              برنامهٔ Enhanced Care برای هر نوع درمان تعداد مشخصی جلسه را در ۱۲ هفتهٔ اول از پیش تأیید می‌کند، بدون نیاز به ارجاع و بدون تأیید کارشناس. چون کلینیک چندتخصصی است، فیزیوتراپیست می‌تواند بیش از یکی از این‌ها را در یک برنامهٔ هماهنگ به کار بگیرد.
            </p>
          </div>
          <dl
          ...
          >
            {preApproved.map((s) => (
              <div key={s.service} className="bg-stone flex items-baseline justify-between gap-4 px-7 py-7">
                <dt className="text-[19px] font-medium text-text leading-tight">{s.service}</dt>
                <dd className="text-[clamp(2.25rem,4vw,3rem)] font-light text-text leading-none shrink-0">{s.sessions}</dd>
              </div>
            ))}
          </dl>
          <p className="text-[14px] text-muted leading-[2] mt-8 max-w-[640px]">
            این‌ها جلساتی است که ICBC از ابتدا تأیید می‌کند. اگر بهبودی شما به جلسات بیشتری نیاز داشته باشد، درمانگرتان از طرف شما درخواست تمدید می‌دهد.
          </p>
        </div>
      </section>

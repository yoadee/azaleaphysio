...
- **SEO pass 1 — MERGED TO PRODUCTION 2026-06-25, now LIVE** (verified: `aggregateRating` gone from live markup, new titles serving). `seo-pass` and `mvp-rebuild` are aligned. Full plan in `SEO-PLAN.md` (output of a 14-agent RampStack SEO orchestration + synthesis + penalty review). Implemented: removed self-serving `aggregateRating` from JSON-LD; practitioner `Physician`→`Person`; fixed invalid location `@type` + dead `SearchAction`; unique metaTitle+metaDescription on all services + expanded static titles (clears Bing duplicate/short flags); hreflang/canonical; `llms.txt` cleanup; **postal code fixed `V6B 5C6`→`V7V 1J6`** (verified for 1884 Marine Dr, was a downtown-Van code).

- **Google Ads plan — BUILT & IMPLEMENTATION-READY (2026-06-25), in `google-ads/`.** Full strategy in `GOOGLE-ADS-STRATEGY.md` + three import files: `azalea-google-ads-BUILD.csv` (Google Ads Editor format: 8 campaigns / 22 ad groups / 97 keywords / 285 RSA headlines, all char-limits validated, UTF-8 BOM for Farsi), `-NEGATIVES.csv` (77, paste into a shared list), `-ASSETS.csv` (7 sitelinks, 10 callouts, 2 snippet sets). Reproducible via `build-generator.mjs` (`node build-generator.mjs`). Ships in a **$200/mo launch state** (ICBC + Farsi + Branded Enabled; rest Paused, ready to flip) and scales to ~$1.5k/mo across 4 tiers. Revenue/ROAS modelled on the real Jane prices (ROAS ~5×–16×). Built with RampStack `paid-media-strategy` / `ads-creative-development` / `seo-keyword` / `seo-competitor`, plus live competitor + CPC research and an adversarial copy + format review. Prereq before spend: mark GA4 `booking_start`/`phone_tap` as conversions and import to Google Ads. NOT yet uploaded (needs Abtin's Google Ads account).
...
**Also live:** **Ahrefs Web Analytics** (cookieless, `next/script`, key `XxUMfSFQI2ISx1b7h7yOuA`) alongside GA4.

**PENDING — pick up tomorrow:**
1. **Live Google rating — INTEGRATION BUILT 2026-06-25, awaiting credentials.** `aggregateRating` stays OUT of JSON-LD (Google policy). The visible rating is now wired to the **Google Places API (New)** via `src/lib/googleRating.ts` (cached 24h, React `cache`), with a graceful fallback to the hardcoded `SITE.googleRating`/`reviewCount` in `clinic.ts`. Consumed by `PageHeader.tsx` (async), `fa/page.tsx` (async, Persian digits), `llms.txt` (async), and the client home `page.tsx` via the new `/api/google-rating` route. **Decision (Abtin): use the 16th Street clinic ONLY** (Ocean Walk has ~3 reviews, would drag the number). **STILL NEEDED from Abtin:** set 2 env vars in Vercel (Prod + Preview) — `GOOGLE_PLACES_API_KEY` (key with "Places API (New)" enabled) and `GOOGLE_PLACE_ID_MAIN` (16th Street Place ID). Until set, site shows the 4.6/83 fallback. Build + tsc clean. NOT committed yet.
2. **Pricing — REAL PRICES NOW CAPTURED (2026-06-25).** Pulled live from the Jane public API (`/api/v2/treatments`), full table in `google-ads/GOOGLE-ADS-STRATEGY.md` Appendix A. Physio **$130 initial / $110 follow-up** (site says $110–135 / $90 — understated). Acupuncture $140/$120, chiro $140/$120, kinesiology $130/$110, massage $130 (45m)/$160 (60m), counselling (ICBC) $268/$160; ICBC + WSBC rates also captured. STILL TODO: align `clinic.ts` + `llms.txt` copy to these. Note: Jane also lists *Laser Skin+Hair* and *Women's Health* disciplines not on the site — confirm if real (cross-sell).

...
1. **Live Google rating (not hardcoded).** `aggregateRating` is OUT of the JSON-LD and staying out (Google policy: schema ratings must be first-party on-page, so do NOT re-add Google's number to schema). But the *visible* "4.6 / 83 Google reviews" text is still hardcoded in `clinic.ts:16-17` and shown in `page.tsx:127`, `PageHeader.tsx:60`, `fa/page.tsx:101`. Abtin opted to defer the live integration. TODO: wire the visible rating to the **Google Places API** (server-side, cached) — needs a Places API key from Abtin + the 2 Place IDs.
